...
export default function Community() {
  ...
  const sendVoiceMessageMutation = useMutation({
    ...
    onError: (error) => {
      toast({ title: "Failed to send voice message", description: error.message, variant: "destructive" });
    },
  });
    ...
    return (
      <ChatLayout showSidebar={false}>
        <div className="flex flex-col items-center justify-center h-screen p-6 bg-chat-bg">
          <Avatar className="h-28 w-28 mb-6 border-4 border-primary/20">
            <AvatarImage src={community.avatar_url || ""} />
            <AvatarFallback className="text-4xl bg-primary/10 text-primary">{community.name[0]}</AvatarFallback>
          </Avatar>
          <h1 className="text-3xl font-bold mb-3 text-foreground">{community.name}</h1>
